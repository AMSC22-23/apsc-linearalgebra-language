...
public:
  ...
  void init(SparseMatrix& A) {
    ...
    for (int i=0; i<M.rows(); i++)
      M.insert(i, i) = static_cast<Scalar>(1);
    ...
    for (; k<M.cols(); k++) {
      ...
      while (residual_norm > tollerance && iter < max_iter) {
        ...
        std::vector<int> L;
        std::vector<int> keepArray(A.cols(), 0); //std::reduce is applied on this vector hence the container scalar type should match the reduce output type
        ...
        for (int i=0; i<A.cols(); i++) {
          for (int j=0; j<l; j++) {
            for (int h=A.outerIndexPtr()[i]; h < A.outerIndexPtr()[i+1]; h++) {
        ...
        for (int i = 0; i < n2Tilde; i++) {
          ...
          for (int j = A.outerIndexPtr()[JTilde[i]]; j < A.outerIndexPtr()[JTilde[i] + 1]; j++) {
          ...
          rhoSq[i] = residual_norm * residual_norm - (rTAe_j * rTAe_j) / (Ae_jNorm * Ae_jNorm);
        }
        ...
        for (int j=0; j<n2Union; j++) {
          for (int i=A.outerIndexPtr()[JUnion[j]]; i<A.outerIndexPtr()[JUnion[j] + 1]; i++) {
            int keep = 1;
            for (int h=0; h<n1; h++) {
              if (A.innerIndexPtr()[i] == I[h] || A.innerIndexPtr()[i] == ITilde[h]) {
                keep = 0;
              }
            }
            if (keep == 1) {
        ...
        update_qr(A, AHat, ADense, Q, R, I, J, sortedJ, ITilde, JTilde, IUnion, JUnion,
  ...
  void set_max_inter(std::size_t iter) {
    max_iter = iter;
  }
...
protected:
  ...
  void solve_manual_least_square(SparseMatrix &A, Vector& b, FullMatrix &Q, FullMatrix &R,
                                Vector &x, int row_size, int k) {
    // 5.1) Compute t = Q^T * b
  ...
  void solve_least_square(FullMatrix &m, Vector &dest, Vector &b, FullMatrix &Q,
                          FullMatrix &R) {
    if constexpr (std::is_base_of_v<Eigen::MatrixBase<FullMatrix>, FullMatrix>) {
      Eigen::HouseholderQR<FullMatrix> qr(m);
      Q = qr.householderQ();
      R = qr.matrixQR().template triangularView<Eigen::Upper>();
      dest = qr.solve(b);
    } else {
      std::cerr << "###### FAILURE: least sqaure problem solver not implemented for the template FullMatrix #####" << std::endl; 
    }
  }

  void create_permutation_matrices(std::vector<int>& I, std::vector<int>& J, int n1, int n2, FullMatrix& Pr, FullMatrix& Pc) {
  ...
  void update_qr(SparseMatrix &A, FullMatrix &AHat, FullMatrix& ADense, FullMatrix &Q, FullMatrix &R,
                std::vector<int> &I, std::vector<int> &J,
                std::vector<int> &sortedJ, std::vector<int> &ITilde,
                std::vector<int> &JTilde, std::vector<int> &IUnion,
                std::vector<int> &JUnion, int n1, int n2, int n1Tilde,
                int n2Tilde, int n1Union, int n2Union, Vector& m_kOut,
                Vector& residual, double& residual_norm, int k) {
