...
int main(int argc, char* argv[]) {
  ...
  if (mpi_rank == 0) {
    std::cout << "Launching BiCGSTAB with a sparse MPI matrix with size: "
  ...
  // Testing sequential BiCGSTAB
  {
    if (mpi_rank == 0) {
      std::cout << "================================= SEQUENTIAL BiCGSTAB "
                   "================================="
                << std::endl
                << std::endl;
    }
    // Test preconditioned BiCGSTAB with SPAI preconditioner
    int r;
    {
      if (mpi_rank == 0) {
        std::cout << "================================= BiCGSTAB with SPAI "
                     "preconditioner ================================="
                  << std::endl;
      }
      EigenVectord x;
      ...
      r = apsc::LinearAlgebra::Utils::Solvers::BiCGSTAB::solve<
          decltype(AM), decltype(b), double, decltype(e), 0>(
          AM, b, y, e, 
          MPIContext(mpi_comm, mpi_rank, mpi_size),
          ObjectiveContext(
              0, mpi_size,
              std::string("sequential_BiCGSTAB_with_precon_matrix_AM") +
    ...
    // Test no preconditioned BiCGSTAB
    {
      if (mpi_rank == 0) {
        std::cout << "================================= BiCGSTAB with no "
                     "preconditioner ================================="
                  << std::endl;
      }
      EigenVectord x;
      r = apsc::LinearAlgebra::Utils::Solvers::BiCGSTAB::solve<
          ...
          ObjectiveContext(
              0, mpi_size,
              std::string("sequential_BiCGSTAB_with_no_precon_matrix_A") +
                  std::string("_epsilon") + std::to_string(tol) +
                  std::string("_MPISIZE") + std::to_string(mpi_size) + ".log",
              std::string(argv[1])));
    }
    ...
    // Test preconditioned BiCGSTAB with SPAI preconditioner
    {
      ...
      // retrive y, the unpreconditioned solver is called as the preconditioner
      // is embedded in the input matrix
      r = apsc::LinearAlgebra::Utils::Solvers::BiCGSTAB::solve_MPI<
          decltype(PAM), decltype(b), double, decltype(e), 0>(
          PAM, b, y, e,
          MPIContext(mpi_comm, mpi_rank, mpi_size),
          ObjectiveContext(0, mpi_size,
                           std::string("parallel_BiCGSTAB_with_precon_matrix_AM") +
                               std::string("_epsilon") + std::to_string(tol) +
                               std::string("_MPISIZE") +
    ...
    // Test no preconditioned BiCGSTAB
    {
      ...
      r = apsc::LinearAlgebra::Utils::Solvers::BiCGSTAB::solve_MPI<
          ...
          ObjectiveContext(
              0, mpi_size,
              std::string("parallel_BiCGSTAB_with_no_precon_matrix_A") +
                  std::string("_epsilon") + std::to_string(tol) +
                  std::string("_MPISIZE") + std::to_string(mpi_size) + ".log",
              std::string(argv[1])));
    }
