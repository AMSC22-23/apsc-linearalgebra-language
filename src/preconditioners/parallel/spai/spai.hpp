...
// class SPAI {
// #warning \
//     "This SPAI implementation does not reproduce a correct result, please use
//     the cspai.hpp version"
//  public:
//   SPAI() {}
...
//       // A(i, J) We initialize I to -1, and the iterate through all elements
//       of
//       // J. Then we iterate through the row indeces of A from the offset J[j]
//       to
//       // J[j] + 1. If the row index is already in I, we dont do anything,
//       else
//       // we add it to I.
//       I = std::vector<int>(A.rows(), -1);
//       n1 = 0;
//       for (int j = 0; j < n2; j++) {
//         for (int i = A.outerIndexPtr()[J[j]]; i < A.outerIndexPtr()[J[j] +
//         1];
//              i++) {
//           int keep = 1;
//           // TODO: can be removed
...
//       // 3) Create Â = A(I, J)
//       // We initialize AHat to zeros. Then we iterate through all indeces of
//       J,
//       // and iterate through all indeces of I. For each of the indices of I
//       and
//       // the indices in the flatRowIndex, we check if they match. If they do,
//       we
...
//       // Compute residual = (A * mHat_k) - e_k, need hardcoded loops as we
//       have
...
//         // 8) Set JTilde to the set of columns of A corresponding to the
//         indices
...
//         // Denote by ITilde the new rows, which corresponds to the nonzero
//         rows
...
//         update_qr(A, AHat, ADense, Q, R, I, J, sortedJ, ITilde, JTilde,
//         IUnion,
...
//         for (int l = m.outerIndexPtr()[J[j]]; l < m.outerIndexPtr()[J[j] +
//         1];
//              l++) {
...
//   void solve_least_square(FullMatrix &m, Vector &dest, Vector &b, FullMatrix
//   &Q,
//                           FullMatrix &R) {
...
//     FullMatrix B2R = B2_qr.matrixQR().template
...
//     // Make second matrix with identity in the upper left corner and B2Q in
//     the
...
//     // Make unsortedR with R in the top left corner, B1 in the top right
//     corner
...
//       }
//     }
//     solve_manual_least_square(A, e_k, unsortedQ, unsortedR, m_kOut, n2Union,
//     k);
