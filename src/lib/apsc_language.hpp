...
class SparseMatrix {
 public:
   ...
   */
  VectorX operator*(VectorX const& vec) {
    if constexpr (USE_MPI) {
      VectorX collection;
  ...
  VectorX solve_iterative(VectorX rhs) {
    VectorX x(rhs.size());
  ...
  /**
   * @brief Solves the linear system directly using QR factorization. Only master rank will collect the result vector.
   * @param rhs Right-hand side vector.
   * @return Solution vector.
   */
  VectorX solve_direct(VectorX& rhs) {
    static_assert(OT == OrderingType::COLUMNMAJOR,
                  "In order to use the QR direct solver the matrix should be "
                  "column ordered");
    // Reset to avoid wrong calls
    solver_iter = 0;
    eigen_sparse_matrix.makeCompressed();
    Eigen::SparseQR<decltype(eigen_sparse_matrix), Eigen::COLAMDOrdering<int>> solver(eigen_sparse_matrix);
    VectorX x;
    if constexpr (USE_MPI) {
      if (mpi_rank == 0) {
        x = solver.solve(rhs);
 ...
 protected:
   ...
   */
  MPISparseMatrix<decltype(eigen_sparse_matrix), VectorX,
...
class FullMatrix {
 public:
  using VectorX = apsc::LinearAlgebra::Vector<Scalar>;
  /**
   ...
   */
  VectorX solve_iterative(VectorX rhs) {
    VectorX x(rhs.size(), static_cast<Scalar>(0.0));
    // Maybe parametrise this
    int max_iter = CG_MAX_ITER(rhs.size());
    Scalar tol = CG_TOL;
 ...
 protected:
   ...
   */
  apsc::LinearAlgebra::MPIFullMatrix<decltype(full_matirx), VectorX,
                                     OT == OrderingType::COLUMNMAJOR
